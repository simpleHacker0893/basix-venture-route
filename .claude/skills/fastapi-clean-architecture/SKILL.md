...
description: >
  Estructura proyectos FastAPI de forma pragmática separando lógica de negocio
  de framework, ORM y servicios externos. Capas Domain/Application/Infrastructure
  cuando paga, no por purismo. Soporta integraciones Celery 5.4, PyMongo async,
  auth Clerk JWT + API keys SHA-256. Modo single-agent para piezas sueltas,
  modo multi-subagent para generar un módulo completo (entities + use cases +
  repositorios + router) en paralelo.
  Trigger: "caso de uso", "use case", "router FastAPI", "clean architecture",
  "repositorio", "lifespan", "desacoplar FastAPI", "Celery task", "PyMongo async",
  "Clerk JWT", "API key".
license: MIT
metadata:
  author: fulit103
