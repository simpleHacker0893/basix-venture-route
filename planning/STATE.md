...
- Evidence: 119 pytest (all five DOMAIN.md scenarios exact over both HTTP paths, byte-identical route between form and chat), 7 Vitest, ruff, mypy strict (`uv run mypy .` in `services/engine`, tests and scripts included) and `pnpm -r typecheck` green on Windows. `/code-review master` applied three fixes: dated `currentBrief` serialised as JSON in the Anthropic adapter (#17), `demoData` on `PartialBrief` so a scenario round-trips as `currentBrief` (#11, #16), and mypy over tests and scripts (#18). The engine agreed with every expected scenario value; no seed was adjusted.
