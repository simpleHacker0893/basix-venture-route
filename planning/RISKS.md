...
| R-04 | Convex + FastAPI dual backend confuses the projection boundary | Medium | Medium | D-15: engine owns the atomspace; Convex owns records; one `reproject` endpoint; full rebuild only | Builder |
...
| R-10 | Clerk + Convex role claims misconfigured, marketplace unusable on demo | Medium | Medium | Sprint 003 acceptance includes a signed-in smoke test per role; `ADMIN_EMAILS` bootstrap | Builder |
