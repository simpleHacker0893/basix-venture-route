...
## In scope
1. `docker compose up` brings up `engine` and `web` with seed data; `README.md` documents prerequisites, `.env.example`, the single launch command, the offline demo flag, and the demo script (five scenarios in order with expected screens).
2. Deploy: engine Docker image to Railway (Render fallback) with `/health`; web to Vercel with `VITE_API_URL`; Convex `npx convex deploy`. Public URLs recorded in README.
